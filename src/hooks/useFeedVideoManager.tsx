...
/**
 * Phase 1 — Single Active Feed Video Manager.
 *
 ...
 *
 * The manager is OPT-IN via <FeedVideoManagerProvider>. Outside a provider,
 * `useFeedVideoSlot` returns `managed: false` and FeedVideo falls back to
 * its legacy `useVideoAutoplay` behavior, so unwrapped surfaces are
 * untouched.
 */
...
const Ctx = createContext<ManagerApi | null>(null);
...
export function FeedVideoManagerProvider({ children }: { children: React.ReactNode }) {
  ...
  const recompute = useCallback(() => {
    ...
    // Pick best by effective ratio; tiebreak by smaller center distance.
    ...
    if (current && slotsRef.current.has(current)) {
      ...
      if (cur.effective >= KEEP_THRESHOLD) {
        // Stay active unless a challenger clearly wins by the switch margin.
        if (
          bestId &&
          bestId !== current &&
      ...
      // Current dropped below KEEP — fall through to reselect.
    }
  ...
  // Observer + scroll/resize/visibility listeners.
  useEffect(() => {
    ...
    // Re-observe any slots registered before the observer existed.
    slotsRef.current.forEach((slot) => {
  ...
  const setLightboxOpen = useCallback(
    (open: boolean) => {
      ...
      if (open) {
        // Synchronous pause: clear active immediately, no debounce.
        if (debounceRef.current !== null) {
          window.clearTimeout(debounceRef.current);
          debounceRef.current = null;
        }
        ...
        // After close, recompute soon to pick the now-visible video.
        flushNow();
      }
    },
...
export function useFeedVideoSlot(id: string): SlotReturn {
  ...
  return {
    managed: !!ctx,
    isActive: ctx ? ctx.activeId === id : false,
    registerEl,
  };
}
...
export function useFeedVideoManagerControls(): ControlsReturn {
  ...
  return {
    isPresent: !!ctx,
    setLightboxOpen: ctx ? ctx.setLightboxOpen : () => {},
  };
}
