  ...
  return <RadixTabs.Root value={activeTab} onValueChange={handleTabChange} className="px-0 py-[96px]">
      <div className="flex justify-center mb-6">
        <RadixTabs.List className={cn("flex items-center gap-2 py-1 px-1 rounded-full shadow-lg", "bg-background/30 border border-white/10 backdrop-blur-md")}>
          ...
          return <RadixTabs.Trigger key={item.value} value={item.value} className={cn("relative cursor-pointer text-sm font-semibold px-6 py-2 rounded-full transition-colors", "text-foreground/80 hover:text-primary flex items-center gap-2", isActive && "text-primary")}>
                {Icon && <Icon size={16} />}
                <span>{item.label}</span>
                
                {isActive && <motion.div layoutId="tube-tab" className="absolute inset-0 w-full bg-primary/10 rounded-full -z-10" initial={false} transition={{
              type: "spring",
              stiffness: 300,
              damping: 30
            }}>
