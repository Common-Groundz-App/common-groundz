...
import { Search, X, ThumbsUp, Filter } from 'lucide-react';
...
import { getEntityRecommendersWithContext, type EntityRecommenderWithContext } from '@/services/entityRecommendationService';
  ...
  const fetchRecommenders = async () => {
    ...
    try {
      const data = await getEntityRecommendersWithContext(entityId, user?.id || null, {
        limit: 100 // Get more recommenders for the modal
      });
      ...
      <DialogContent className="max-w-md h-[80vh] flex flex-col">
        <DialogHeader className="flex-shrink-0">
          ...
          <p className="text-sm text-muted-foreground">
            {totalRecommendationCount} recommender{totalRecommendationCount !== 1 ? 's' : ''}
            {followingCount > 0 && (
              ...
              <div className="divide-y divide-gray-100">
                {filteredRecommenders.map((recommender) => (
                  <div key={recommender.id} className="transition-all duration-200">
                    ...
                    />
                    {/* Relationship context and rating */}
