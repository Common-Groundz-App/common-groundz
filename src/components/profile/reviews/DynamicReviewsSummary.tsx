...
interface DynamicReviewsSummaryProps {
  ...
  timelineData?: {
    ...
    // Entity-level AI summary data (not from individual reviews)
...
export const DynamicReviewsSummary = ({ 
  ...
  const renderEntityAISummary = () => {
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1 space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  {timelineData.entityAiSummaryLastGenerated && (
                    <Tooltip>
                      <TooltipTrigger asChild>
                        <div className="text-xs text-violet-600 dark:text-violet-400 bg-violet-100 dark:bg-violet-900/30 px-2 py-0.5 rounded-full">
                          {timelineData.entityAiSummaryModel === 'gemini-1.5-flash' ? 'Gemini' : 'GPT'}
                        </div>
                      </TooltipTrigger>
                      <TooltipContent>
                        <p className="text-xs">
                          Generated {formatRelativeDate(timelineData.entityAiSummaryLastGenerated)} using {timelineData.entityAiSummaryModel}
                        </p>
                      </TooltipContent>
                    </Tooltip>
                  )}
    ...
    // Show placeholder only when there are multiple dynamic reviews but no entity-level summary yet
    if (dynamicReviews.length >= 2) {
      return (
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3 text-violet-600 dark:text-violet-400">
              ...
              <div className="flex-1">
                <div className="font-medium text-sm mb-1">Entity-level AI Insights</div>
                <div className="text-xs text-muted-foreground">
                  Aggregated insights across all dynamic reviews will appear here when generated by admins.
                </div>
              </div>
            </div>
          </CardContent>
    ...
    // Fallback for when there aren't enough reviews for entity-level insights
    return (
          ...
          <div className="flex items-center gap-3 text-muted-foreground">
            ...
            <div className="flex-1">
              ...
              <div className="text-xs">
                Entity-level insights will appear here after multiple users share timeline updates.
              </div>
            </div>
          </div>
        </CardContent>
  ...
  return (
    <TooltipProvider>
      <div className="space-y-4 mb-6">
        {/* Entity-level AI Summary Card - Only shows entity summaries */}
        {renderEntityAISummary()}
