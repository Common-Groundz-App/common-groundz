/**
 * Phase 2.1 contract test.
 *
 * Edge functions cannot import from `src/`, so `supabase/functions/_shared/
 * reviewCategoryBuckets.ts` mirrors the frontend canonical→bucket mapping.
 * This test makes the mirror enforced rather than aspirational: adding or
 * changing a canonical type on one side without the other fails here.
 */
