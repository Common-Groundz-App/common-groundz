...
DROP FUNCTION IF EXISTS public.get_aggregated_network_recommendations_discovery(uuid, integer);
DROP FUNCTION IF EXISTS public.get_aggregated_network_recommendations_discovery(uuid, uuid, integer);

-- Recreate with correct signature: queries reviews table, accepts p_entity_id, returns correct column names
