...
-- v8c — Google CSE image fallback for Search-to-Draft (Vertex rows only).
-- Mirrors the v8b Firecrawl migration structure exactly.
...
VALUES (
  ...
  '{"enabled": false}'::jsonb,
  'When true, enrich-candidate-image may run one Google Custom Search image query as a last-resort fallback for Vertex-interstitial search results (vertexaisearch.cloud.google.com). Skips Firecrawl for those rows. Search-to-Draft only; URL Analysis unaffected.'
)
...
--    from the v8b migration (20260718080128_...sql). Only two edits:
--      - key added to the IN-list.
--      - new ELSIF branch appended for the same shape { "enabled": boolean }.
