...
CREATE TABLE public.post_user_mentions (
...
CREATE INDEX idx_post_user_mentions_post_id ON public.post_user_mentions(post_id);
CREATE INDEX idx_post_user_mentions_mentioned_user_id ON public.post_user_mentions(mentioned_user_id);

-- Policies
-- Anyone can view mentions for public posts (and not deleted)
CREATE POLICY "View mentions on public posts"
...
CREATE POLICY "View mentions on circle posts with access"
ON public.post_user_mentions
FOR SELECT
USING (
  EXISTS (
...
CREATE POLICY "View mentions on own private posts"
ON public.post_user_mentions
FOR SELECT
USING (
  EXISTS (
...
CREATE POLICY "Insert mentions for own posts"
ON public.post_user_mentions
FOR INSERT
WITH CHECK (
  EXISTS (
...
CREATE POLICY "Delete mentions for own posts"
ON public.post_user_mentions
FOR DELETE
USING (
  EXISTS (
