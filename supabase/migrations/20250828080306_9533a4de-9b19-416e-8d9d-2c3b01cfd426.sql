-- Fix the network recommendations function to show OTHER entities recommended by your network
-- The key fix: Change WHERE r.entity_id = p_entity_id to WHERE r.entity_id != p_entity_id
...
BEGIN
  ...
  network_reviews AS (
    ...
    WHERE r.entity_id != p_entity_id  -- FIXED: Show OTHER entities, not the current one
      AND r.is_recommended = true
      AND r.has_timeline = true
  ...
  aggregated_data AS (
    SELECT 
      ...
      ARRAY_AGG(nr.username ORDER BY nr.created_at DESC) as recommender_names,
      ARRAY_AGG(nr.user_id ORDER BY nr.created_at DESC) as recommender_ids,
