-- Drop existing function since return type is changing
...
-- Recreate with first_name and last_name in return type
...
AS $$
  ...
  friends_of_friends AS (
    ...
    JOIN public.follows f2 ON f2.following_id = p.id
    JOIN public.follows f1 ON f1.following_id = f2.follower_id
  ...
  active_creators AS (
    ...
    FROM public.profiles p
    JOIN (
      SELECT user_id, created_at FROM public.posts WHERE created_at >= now() - interval '7 days'
      UNION ALL
      SELECT user_id, created_at FROM public.recommendations WHERE created_at >= now() - interval '7 days'
