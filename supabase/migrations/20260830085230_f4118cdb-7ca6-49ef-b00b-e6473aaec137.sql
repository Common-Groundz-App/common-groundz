-- 1) create_entity_subject: allow standalone `service`
CREATE OR REPLACE FUNCTION public.create_entity_subject(p_name text, p_type text, p_parent_id uuid DEFAULT NULL::uuid, p_api_source text DEFAULT NULL::text, p_api_ref text DEFAULT NULL::text, p_website_url text DEFAULT NULL::text, p_metadata jsonb DEFAULT '{}'::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
DECLARE
  ...
  -- `service` is the service itself (Haircut, AC repair), never the provider.
  v_standalone   text[] := ARRAY[
    'place','book','movie','tv_show','course','app','game',
    'event','brand','professional','experience','product','service','others'
  ];
...
-- 2) Review subject invariant (INSERT strict, UPDATE transition-aware)
CREATE OR REPLACE FUNCTION public.reviews_enforce_subject()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    IF NEW.entity_id IS NULL THEN
      RAISE EXCEPTION 'A review must be linked to what it is about.'
        USING ERRCODE = '23514';
    END IF;
    v_must_validate := true;
  ELSE
    -- A linked review can never become unlinked.
    IF OLD.entity_id IS NOT NULL AND NEW.entity_id IS NULL THEN
      RAISE EXCEPTION 'This review is already linked to a subject and cannot be unlinked.'
        USING ERRCODE = '23514';
    END IF;

    -- Legacy unlinked rows may stay unlinked.
    IF NEW.entity_id IS NULL THEN
      RETURN NEW;
    END IF;

    -- Validate only when the subject or the category actually changes, so
    -- pre-existing mismatched rows can keep editing unrelated fields.
    v_must_validate := (NEW.entity_id IS DISTINCT FROM OLD.entity_id)
  ...
  IF v_entity.id IS NULL OR v_entity.is_deleted THEN
    RAISE EXCEPTION 'The subject of this review no longer exists.'
      USING ERRCODE = '23503';
  END IF;

  -- NULL-safe: a null/blank category must never pass through unknown logic.
  IF NEW.category IS DISTINCT FROM v_entity.type THEN
    RAISE EXCEPTION 'Review category (%) must match the subject type (%).',
      coalesce(NEW.category, 'none'), v_entity.type USING ERRCODE = '23514';
  END IF;
...
ALTER FUNCTION public.reviews_enforce_subject() OWNER TO postgres;
REVOKE EXECUTE ON FUNCTION public.reviews_enforce_subject() FROM PUBLIC;
REVOKE EXECUTE ON FUNCTION public.reviews_enforce_subject() FROM anon;
REVOKE EXECUTE ON FUNCTION public.reviews_enforce_subject() FROM authenticated;

DROP TRIGGER IF EXISTS reviews_enforce_subject ON public.reviews;
CREATE TRIGGER reviews_enforce_subject
BEFORE INSERT OR UPDATE ON public.reviews
FOR EACH ROW EXECUTE FUNCTION public.reviews_enforce_subject();