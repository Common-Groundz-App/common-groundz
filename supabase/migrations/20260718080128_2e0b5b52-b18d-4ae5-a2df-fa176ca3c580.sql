...
-- v8b — Firecrawl fallback for search-image enrichment.
...
VALUES (
  ...
  '{"enabled": false}'::jsonb,
  'When true, enrich-candidate-image may use Firecrawl as a last-resort fallback for Google/Vertex interstitial search results that fail direct fetch. Search-to-Draft only; URL Analysis unaffected.'
)
ON CONFLICT (key) DO NOTHING;

-- 2. Read helper.
CREATE OR REPLACE FUNCTION public.is_search_image_firecrawl_enabled()
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT COALESCE(
    (SELECT (value->>'enabled')::boolean
      ...
      WHERE key = 'entity_extraction.search_image_firecrawl_enabled'),
    false
  );
$$;
GRANT EXECUTE ON FUNCTION public.is_search_image_firecrawl_enabled() TO anon, authenticated, service_role;
