...
-- v8e — Brand logo lookup for Search-to-Draft rows. Off by default.
-- Mirrors the v8b/v8c migration structure exactly.
...
VALUES (
  ...
  '{"enabled": false}'::jsonb,
  'When true, Search-to-Draft results fetch brand logos in the background via the resolve-brand-logo edge function (same Google CSE lookup used by URL Analysis). Admins can test even when OFF. Uses the existing GOOGLE_CUSTOM_SEARCH_API_KEY / GOOGLE_CUSTOM_SEARCH_CX quota.'
)
ON CONFLICT (key) DO NOTHING;

-- 2. Read helper.
CREATE OR REPLACE FUNCTION public.is_search_brand_logo_lookup_enabled()
RETURNS boolean
LANGUAGE sql
STABLE
SECURITY DEFINER
...
AS $$
  SELECT COALESCE(
    (SELECT (value->>'enabled')::boolean
      ...
      WHERE key = 'entity_extraction.search_brand_logo_lookup_enabled'),
    false
  );
$$;
GRANT EXECUTE ON FUNCTION public.is_search_brand_logo_lookup_enabled() TO anon, authenticated, service_role;

-- 3. Extend set_app_flag allowlist. Preserves every existing branch verbatim
--    from the v8c migration (20260720050453_...sql). Only two edits:
--      - key added to the IN-list.
--      - new ELSIF branch appended for the same { "enabled": boolean } shape.
