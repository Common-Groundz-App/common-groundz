-- Add unique constraint to prevent duplicate photo caching
ALTER TABLE public.cached_photos 
ADD CONSTRAINT unique_entity_original_url 
UNIQUE (entity_id, original_url);

-- Clean up existing duplicates (keep the most recent one for each entity_id + original_url combination)
