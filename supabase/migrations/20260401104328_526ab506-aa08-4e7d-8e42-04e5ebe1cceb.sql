...
-- Revoke direct writes from client roles — only SECURITY DEFINER RPCs can write
REVOKE INSERT, UPDATE, DELETE ON public.comment_mentions FROM anon, authenticated;

CREATE INDEX idx_comment_mentions_comment ON public.comment_mentions(comment_id, comment_type);
CREATE INDEX idx_comment_mentions_user ON public.comment_mentions(mentioned_user_id);

-- ============================================================
-- 2. Notification dedup performance index
-- ============================================================
CREATE INDEX IF NOT EXISTS idx_notifications_dedup
  ON public.notifications (user_id, sender_id, entity_id, entity_type);

-- ============================================================
-- 3. Updated add_comment RPC with mention parsing & notifications
-- ============================================================
...
BEGIN
  ...
  -- Update comment count on parent item
  EXECUTE format('
    UPDATE %I
  ...
  -- Get commenter username for notification messages
  ...
  -- ============================================================
  -- Parse @mentions (PostgreSQL-safe: no lookbehind)
  -- Pattern: start-of-string or non-identifier char, then @username
  -- ============================================================
  ...
  LOOP
    ...
    -- Skip duplicates within this comment
    ...
    -- Insert mention notification (with dedup)
  ...
  IF p_parent_id IS NOT NULL THEN
    ...
    -- Notify parent author (skip self, skip if already mentioned)
    IF parent_author_id IS NOT NULL
       AND parent_author_id <> p_user_id
       AND NOT (parent_author_id = ANY(
         SELECT unnest(ARRAY(
           SELECT cm.mentioned_user_id FROM public.comment_mentions cm
           WHERE cm.comment_id = new_comment_id AND cm.comment_type = p_item_type
         ))
       ))
    THEN
      INSERT INTO public.notifications (user_id, sender_id, type, title, message, entity_id, entity_type, action_url, metadata)
...
-- ============================================================
-- 4. Updated toggle_comment_like RPC with like notifications
-- ============================================================
...
BEGIN
  ...
  ELSE
    ...
    -- Get comment author and parent item ID for notification
    ...
    IF comment_author_id IS NOT NULL AND comment_author_id <> p_user_id THEN
      ...
      -- Insert like notification with dedup
      INSERT INTO public.notifications (user_id, sender_id, type, title, message, entity_id, entity_type, action_url, metadata)
      SELECT
        comment_author_id,
        p_user_id,
