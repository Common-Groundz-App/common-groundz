...
function logPerformanceMetric(url: string, metric: RequestMetric) {
  ...
  console.log(`🎬 ${status} ${cache} ${metric.duration}ms ${sizeMB}MB - Movie: ${url}`)
  
  if (!metric.success) {
    console.error(`❌ Movie proxy failed for: ${url}`)
  }
}
...
serve(async (req) => {
  ...
  try {
    ...
    console.log('🔍 Fetching movie image:', imageUrl)
    ...
    if (!response.ok) {
      console.error('Failed to fetch movie image:', response.status, response.statusText)
      metric.duration = Date.now() - startTime
      logPerformanceMetric(imageUrl, metric)
    ...
    console.log(`✅ Successfully served movie image: ${uint8Array.length} bytes (${contentType})`)
