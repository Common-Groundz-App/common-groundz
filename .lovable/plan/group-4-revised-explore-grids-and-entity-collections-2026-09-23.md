...
One more safeguard worth having: Category highlights has three separate picture blocks in one file and Explore also renders it four more times with a type filter. The focused tests will cover all three branches individually, not just the first — an easy place for one branch to be missed silently. The catch-all guard is narrower than I first wrote it, and the correction is right: a blanket "no Unsplash address" check would contradict the established policy. The guard is that no surface renders a *registered* legacy-placeholder address and none introduces a stock address as a fallback — while a legitimate, unregistered Unsplash photo that is the surface's current real source must keep rendering normally. Both cases get their own explicit test.
