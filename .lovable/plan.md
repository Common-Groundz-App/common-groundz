...
- **Lifecycle identity (a keyed internal attempt component):**
  - The outer `ImageWithFallback` normalises `src` and `fallbackSrc`, derives `lifecycleKey = JSON.stringify([normalizedSrc, normalizedFallbackSrc])`, and renders an internal `<ImageAttempt key={lifecycleKey} … />`.
  - `ImageAttempt` owns the relay/direct attempt state, the fallback state and the once-only callback guards. A change of source or fallback remounts it, so every piece of state resets synchronously, with no render-time ref mutation and no reset effect.
  - Inside `ImageAttempt`, an unmounted guard ignores any event that arrives after it has been replaced.
