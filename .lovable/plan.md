...
`prune_retracted_notifications(p_limit int default 5000)` deletes a bounded batch selected with `ORDER BY retracted_at, id` — both columns, matching the cleanup index, so rows sharing a `retracted_at` value cannot stall progress — where `retracted_at < now() - interval '60 days'`, returning the delete count for monitoring. Never one unbounded DELETE — that means heavy WAL and long locks as the table grows. `EXECUTE` granted to `service_role` only. Scheduled idempotently: `cron.unschedule` guarded by existence, then `cron.schedule` under a fixed job name, so re-running the migration cannot stack duplicate jobs; if the cron extension is unavailable the schedule block is skipped with a notice rather than failing the migration, since retention is not a correctness dependency. Retracted rows are never returned to a user or counted at any point in their life; the window exists purely for debugging and abuse analysis.

## Step 6 — Tests

Extend `notificationRealtime.test.ts`: a retraction UPDATE removes rather than patches; a retracted INSERT is refused; `retracted_at` validation accepts null/ISO and rejects garbage; a late-resolving read mutation cannot restore a retracted id; a burst of retractions coalesces to exactly one reconcile. New coverage for the coordinated both-lane membership pass dropping a retracted older All-lane row, and for the all-or-nothing chunk contract. All 138 existing tests stay green.

## Step 6b — Database-boundary verification

The highest-risk half of this phase lives in PostgreSQL, where TypeScript tests prove nothing. A SQL verification pass (run through the read/insert tooling against seeded scratch rows, cleaned up afterwards) will confirm, before the manual walkthrough: the partial-index conflict targets actually infer (an insert that should no-op does); two rapid identical likes leave exactly one active row; an unlike retracts the correct recipient's row and no other; the backfill mapped stale rows correctly (no active like row without a live source row); the comment soft-delete trigger fires on a real `false → true` transition and retracts every related comment/like row while leaving a `system` row untouched; the retraction triggers work when the *actor*, not the recipient, performs the undo (the `SECURITY DEFINER` requirement); and `prune_retracted_notifications` runs under `service_role` and returns a count.
