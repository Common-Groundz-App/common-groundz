...
- **Duplicate is success, via insert-ignore.** If the row already exists (another surface followed first, or a double click), it resolves to `Following` with no error toast. The insert is `.upsert(row, { onConflict: 'follower_id,following_id', ignoreDuplicates: true })` — `ignoreDuplicates` maps to `ON CONFLICT DO NOTHING`, not `DO UPDATE`. This matters: **verified live, `public.follows` has only INSERT, DELETE and SELECT policies and no UPDATE policy**, so a default upsert would be blocked by RLS on the duplicate path. `DO NOTHING` also avoids touching `created_at` and avoids any UPDATE-side effects.
- **Uniqueness is real, and now also reproducible.** Verified live: `follows_follower_id_following_id_key` is a UNIQUE index on `(follower_id, following_id)`, so duplicates genuinely conflict rather than writing a second row — no behavioural migration is needed for production. Codex only saw the separate non-unique `idx_follows_follower_following` in the migration files, and that gap is worth closing: a small idempotent migration adds `CREATE UNIQUE INDEX IF NOT EXISTS follows_follower_id_following_id_key ON public.follows (follower_id, following_id)` so a scratch database built from migrations gets the same guarantee. It is a no-op against the live database.
