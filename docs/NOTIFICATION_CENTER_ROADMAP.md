...
## Phase 3.3A — Target thumbnails (done)

Read-only preview of the content a notification points at. No migration, no edge function, no producer, and nothing touching realtime, counts, cursors, read state, retraction or preferences.

- **`notifications.image_url` is never used** as content media — it holds the *actor's* avatar, so rendering it would show the same face twice.
- **Eligibility is target-based, not type-based:** any row with `entity_type` `post`/`recommendation` and a valid UUID `entity_id` gets a preview — likes, comments, replies, mentions and comment-likes alike, since comment rows carry the parent's `entity_id` and use `metadata.comment_id` only to locate the comment inside it. Follow rows (`entity_type='profile'`) and system rows are excluded by the same rule.
- **`src/utils/notificationThumbnail.ts`** — pure resolver (no supabase, no React, and deliberately no `imageUtils` import, which pulls in the storage client). Posts resolve from `posts.media` in `order`, skipping `is_deleted` items; recommendations from `recommendations.image_url`.
- **Never a raw video URL.** `muxPosterUrl()` falls back to `thumbnail_url ?? url`, and `url` can be the video file itself, so Mux items go through `muxThumbnailUrl(playback_id)` directly. Legacy (non-Mux) video qualifies only via an explicit `thumbnail_url`; errored Mux items resolve to nothing.
- **Validation:** parseable `http`/`https` only — `data:`, `blob:`, relative and junk values resolve to `null`. Malformed/stringified/empty jsonb resolves to `null` instead of throwing.
- **`src/hooks/notifications/useNotificationTargets.ts`** — one query family per entity type over the distinct targets currently loaded, sorted ids, chunked at 200, so `ceil(unique/200)` bounded requests per type (in practice one). Aggregated groups share a target, so it is one lookup per target, not per row. The react-query key carries `user.id` because RLS visibility is account-specific: a thumbnail resolved for one account can never surface for another.
- **UI:** 40×40 decorative `aria-hidden` `<img>` inside the existing row button (no nested interactive element — the row refactor stays in 3.3B), `loading="lazy"`, `decoding="async"`, `onError` collapses the slot. The slot is reserved only while a known target is still resolving; once it settles with no media the slot is removed and the text column reclaims the width. No placeholder box. Read state moved beside the timestamp (unread dot / read check) so the right edge belongs to the thumbnail.
- **Verified:** 207 unit tests pass, covering image media, Mux ready/preparing/errored, legacy video with and without poster, deleted/empty/malformed media, non-http URLs, comment-scoped eligibility, follow exclusion, dedup, and the 200-id chunk boundary.

## Phase 3.3B — Follow back (done)

Contextual action on follow rows. No migration beyond an idempotent index re-declaration, no producer, no trigger, and nothing touching realtime, counts, cursors, read state, retraction or preferences.

- **Row structure refactor (the prerequisite):** `NotificationRow` was a single `<button>` wrapping the whole row. It is now a non-interactive `<div>` containing (a) an absolutely positioned, full-surface navigation `<button>` at `z-0`, labelled `Open notification: <row sentence>` so it is never an unlabelled hit area, and (b) a pointer-transparent content layer whose action slot re-enables pointer events at `z-10`. A button inside a button is invalid HTML and `stopPropagation` does not repair it. Navigation, read-on-click and keyboard access are unchanged; the row is one Tab stop plus one for the action.
- **`src/utils/notificationFollowBack.ts`** — pure eligibility (no React, no supabase). Single `type='follow'` rows only, `entity_type` `profile` or absent, `sender_id` a valid UUID that is not the viewer, and a signed-in viewer. Aggregated groups are excluded: one action slot cannot represent several actors. Distinct actor ids are returned sorted so identical loaded sets produce identical cache keys, and chunked at 200.
- **`src/hooks/notifications/useFollowBackState.ts`** — the drawer's single follow authority. `useUserFollowing` collapses loading *and* error into `[]`, which makes "not following" indistinguishable from "we don't know" and is exactly what mislabels an already-followed actor; this hook exposes a real tri-state instead. A pending **or errored** chunk leaves its ids `unknown`, and nothing renders for `unknown`.
- **Account safety:** `user.id` is part of every react-query key (follow state is per-account), the viewer id is captured before each mutation and re-checked after via a live ref, so a sign-out or account switch mid-flight neither toasts nor writes stale state, and optimistic/pending maps are dropped on viewer change.
- **Idempotent write:** `upsert(..., { onConflict: 'follower_id,following_id', ignoreDuplicates: true })` → `ON CONFLICT DO NOTHING`. `public.follows` has only INSERT/DELETE/SELECT policies, so a default upsert (DO UPDATE) would fail; DO NOTHING also avoids touching an existing row's timestamp. One in-flight follow per actor, guarded by a ref so repeated presses coalesce without waiting for a state flush.
- **Not a toggle:** the drawer can follow, never unfollow. `following` renders as muted status text, not a focusable fake-disabled button.
- **Gates:** `requireAuth({ action: 'follow', surface: 'notification_drawer' })` first, then `canPerformAction('canFollowUsers')` — the same order as `use-follow.ts`.
- **Cache fan-out:** updates its own chunk cache, invalidates `notification-follow-back` / `followers` / `following`, patches `user-following`, and dispatches `follow-status-changed` for the non-react-query consumers (profile header counts).
- **DB:** `CREATE UNIQUE INDEX IF NOT EXISTS follows_follower_id_following_id_key ON public.follows (follower_id, following_id)` — the index already existed live; the migration only brings the repo in sync so `onConflict` is reproducible.
- **Verified:** 220 unit tests pass (13 new), covering aggregated exclusion, non-follow types, follow rows pointing at non-profile content, self-follow, malformed/missing actor ids, signed-out viewers, dedup + sort, and the chunk boundary.
